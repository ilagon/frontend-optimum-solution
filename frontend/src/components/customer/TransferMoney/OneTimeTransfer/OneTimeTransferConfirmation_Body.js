...
export default function BodyContainer() {
  ...
  console.log("cc id:" + state.transfer.creditCard._id);
  const handleSubmit = () => {
    axios
      .post("http://localhost:9000/payment_history/addPayment", {
        payment_type: "Transfer",
        payment_amount: state.transfer.amount,
        transfer_number: 1,
        creditcardId: state.transfer.creditCard._id,
      })
      .then((response) => {
        ...
        axios
          .patch("http://localhost:9000/creditcard/updateBalance", {
            creditcardId: state.transfer.creditCard._id,
            creditcard_balance:
              state.transfer.creditCard.creditcard_balance -
  ...
  const formTo = (
    <div className="toForm toFormConfirmation">
      <h1>To</h1>
      <p>Reference Number: {state.transfer.payeeInfo.number}</p>
  ...
  const formFrom = (
    <div className="fromForm fromFormConfirmation">
      <h1>From</h1>
      <p>{state.transfer.creditCard.creditcard_type}</p>
      <div>
        <Button
          id="submitButton"
          variant="contained"
